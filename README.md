...
- 새가족 시트 동기화는 고정 A/B/C 열 위치 대신 실제 한/영 헤더를 기준으로 필드를 읽습니다. 생년월일/첫 방문일의 잘못된 값은 표시하지 않고, 만 나이는 생년월일로 계산합니다. 기존 데이터는 배포 후 새가족 페이지에서 동기화를 한 번 실행해 갱신할 수 있습니다. 이 수정에는 SQL 변경이 없습니다.

## 공개 링크

- `/links`는 로그인 없이 읽을 수 있습니다. 익명 메뉴에는 오늘의 기도회와 링크만 표시됩니다.
- 배포 전에 Supabase SQL Editor에서 `db/043_public_important_links.sql`을 실행하세요. 신규 환경은 `db/schema.sql`에 포함되어 있습니다.
- 공개 RPC는 제목/설명/URL/아이콘/링크 ID만 반환합니다. 작성자 정보와 멤버 roster는 공개하지 않으며 기존 추가(순장 이상)/삭제(관리자 이상) 권한 및 감사 로그를 유지합니다. 등록된 모든 링크의 제목, 설명, URL이 공개되므로 내부 전용 주소나 비밀 토큰이 포함된 링크는 등록하지 마세요.
- `test/sql/public-links.sql`은 빈 임시 PostgreSQL에서만 실행하는 공개 조회/익명 쓰기 차단 통합 테스트입니다. 운영 DB에서 실행하지 마세요. `/design-preview?view=public-links`는 개발 전용 샘플 화면입니다.
