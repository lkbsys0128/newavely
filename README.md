...
- 공통 색상은 Vanilla Cream `#EDE4D5`, Dark Jade `#163C32`를 기준으로 합니다. 선택 상태와 주요 버튼은 라이트모드에서 제이드 배경/바닐라 글자, 다크모드에서 바닐라 배경/제이드 글자를 사용합니다. Hover는 옅은 제이드 계열로 통일합니다.
...
- 전체 폰트 굵기와 테마는 `src/app/autumn-theme.css`의 공통 토큰을 사용합니다. 제이드 로고는 `public/newave-icon-jade.png`를 직접 사용하며 `seasonal-logo`는 다크모드에서 바닐라 톤으로 보정합니다. 기존 파란 로고는 원본으로 보존합니다. `/design-preview?view=members`에서 멤버 화면을 확인할 수 있으며 `attendance`, `calendar`, `groups`, `new-family`도 지원합니다.
