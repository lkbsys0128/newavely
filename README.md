...
- 상세 출석 통계는 기본으로 최근 한 달(4주)을 표시하며 시작일/종료일을 포함하는 범위로 조회합니다. 최근 4주/12주는 마지막 출석 이벤트 날짜 기준이며, 전체 기간과 직접 지정도 지원합니다.
- 라인 그래프는 순 인원 기준 전체 출석(예배 또는 순모임, 날짜별 중복 제외) 한 개 선만 표시합니다. 방문자/새가족 등 추가 출석 집계는 포함하지 않습니다. 이벤트 종류 선택은 누적 통계에 적용하며 그래프는 전체 출석을 유지합니다.
