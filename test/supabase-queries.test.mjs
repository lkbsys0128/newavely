...
test("dashboard metric cards use role-independent server metrics", () => {
  ...
  assert.match(appPageDataSource, /needsPublicDashboard = options.page === "attendance" \|\| !hasPermission\(currentMember\.role, "members:write"\)/);
  assert.match(appPageDataSource, /\[dashboardData, sharedDashboardData\] = await Promise.all/);
  assert.match(appPageDataSource, /getPublicDashboardData\(supabase, options\?\.attendanceEventId\)/);
  assert.match(appPageDataSource, /publicDashboardData = sharedDashboardData \?\? dashboardData/);
...
test("new family applicants sync from Google Sheets into a role-gated roster", () => {
  ...
  assert.match(newFamilySyncSource, /canonicalizeNewFamilyFields/);
  assert.doesNotMatch(newFamilySyncSource, /newFamilySheetColumnIndexes|pickCell/);
  ...
  assert.match(dashboardSource, /지인 이름/);
  assert.match(dashboardSource, /사용 언어/);
  assert.match(dashboardSource, /개인정보 동의/);
  assert.match(dashboardSource, /라이드 필요/);
  ...
  assert.match(actionsSource, /new_family_first_visit_date/);
  assert.match(actionsSource, /new_family_visit_purpose/);
  assert.match(actionsSource, /new_family_ride_needed/);
