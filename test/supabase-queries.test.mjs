...
test("group page renders a central Newavely network map", () => {
  ...
  assert.match(dashboardSource, /src="\/newave-icon-autumn\.png"/);
...
test("new family applicants sync from Google Sheets into a role-gated roster", () => {
  ...
  assert.match(newFamilySyncSource, /visitPurpose: 9/);
  assert.match(newFamilySyncSource, /visitPath: 10/);
  assert.match(newFamilySyncSource, /referrerName: 11/);
  assert.match(newFamilySyncSource, /preferredLanguage: 12/);
