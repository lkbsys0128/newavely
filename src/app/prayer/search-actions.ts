...
export async function searchPrayerSources(input: unknown): Promise<{ results: PrayerSourceResult[]; error?: string }> {
  ...
  try {
    ...
    if (!clientId || !clientSecret) return { results: [], error: "네이버 검색 API가 아직 설정되지 않았습니다. 관리자에게 문의해주세요." };
    ...
    return { results: await fetchPrayerSources(input.trim(), { clientId, clientSecret }) };
