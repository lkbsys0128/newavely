...
type Props = { sourceUrl?: string; disabled: boolean; label: string; onSelect: (url: string, songTitle?: string) => void };
...
export function PrayerSourcePicker({ sourceUrl, disabled, label, onSelect }: Props) {
  ...
  async function search() {
    ...
    const songTitle = query.trim();
    setLoading(true); setMessage(""); setResults([]);
    try {
      const result = await searchPrayerSources(songTitle);
      if (id !== requestId.current) return;
      setResults(result.results);
      setSearchedTitle(songTitle);
  ...
  return <div className="prayer-source-picker">
    <div className="prayer-source-query"><label>가사 원문 검색<input aria-label={`${label} 가사 원문 검색`} value={query} maxLength={120} placeholder="찬양 제목" disabled={disabled}
    ...
    {results.length ? <ul className="prayer-source-results" aria-label={`${label} 검색 결과`}>{results.map((result) => <li key={result.url}>
      <button type="button" disabled={disabled} onClick={() => { onSelect(result.url, searchedTitle); setResults([]); setMessage("원문 링크를 첨부했습니다."); }}><Link2 size={16} /><span>{result.title}<small>{result.domain}</small></span></button>
