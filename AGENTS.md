...
- 출석 토글은 `useOptimistic`으로 서버 props 위에 변경을 반영합니다. 멤버별 ref 잠금을 서버 응답뿐 아니라 React transition 완료까지 유지하고, 다른 멤버는 연속 체크할 수 있게 합니다. 실패 시 해당 이벤트만 복구하며 전체 명단을 이전 snapshot으로 되돌리지 않습니다. `모두` 성공 후 중복 `router.refresh()`를 추가하지 않습니다. 서로 다른 브라우저의 동시 수정은 기존 서버의 마지막 저장 우선 정책을 유지합니다.
