...
- 브랜드 기본 팔레트는 Vanilla Cream `#EDE4D5`, Dark Jade `#163C32`입니다. 라벤더/가넷 브랜드 강조색은 사용하지 않습니다. `autumn-theme.css`의 `--palette-*` 토큰이 기준이며, 선택/주요 버튼은 라이트에서 제이드 배경과 바닐라 글자, 다크에서 반대로 사용합니다. Hover 글자와 선택 글자는 별도 토큰으로 대비를 유지합니다.
- 폰트 굵기는 `--font-weight-ui`(650), 색상은 `src/app/autumn-theme.css`의 공통 토큰을 사용합니다. 새 화면에 800~950 굵기나 청록색 하드코딩을 추가하지 않습니다. 완료/경고/미출석 같은 의미 있는 상태 색상은 유지합니다. 뉴웨이브 로고는 `/newave-icon-jade.png`와 `seasonal-logo` 클래스를 사용하며 기존 파란 원본 PNG는 보존합니다.
