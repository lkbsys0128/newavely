...
- 상세 출석 통계는 시작/종료일 포함 범위로 조회합니다. `attendance-trend.ts`는 날짜별 예배/순모임 중복 출석을 한 명으로 계산하며 이벤트 중복도 제거합니다. 공통 그래프의 `dailyGroupTrend`는 서버의 통계 대상 명단으로 만들고, 역할별 raw 명단으로 덮어쓰지 않습니다. 방문자/새가족 수기 집계와 순 인원 추이는 구분하며 이벤트 없음(null)과 출석 0명을 구분합니다.
