...
- 폰트 굵기는 `--font-weight-ui`(650), 색상은 `src/app/autumn-theme.css`의 공통 토큰을 사용합니다. 새 화면에 800~950 굵기나 청록색 하드코딩을 추가하지 않습니다. 완료/경고/미출석 같은 의미 있는 상태 색상은 유지합니다. 뉴웨이브 로고에는 `seasonal-logo` 클래스를 사용하며 원본 PNG는 보존합니다.
